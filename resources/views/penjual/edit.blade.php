...
<head>
    ...
    <title>Edit Hewan Qurban | FarmQ Vendor Panel</title>
...
<body class="bg-gray-50 text-gray-800 antialiased min-h-screen pb-12 selection:bg-green-500 selection:text-white">
    ...
    <div class="max-w-2xl mx-auto bg-white p-6 md:p-10 rounded-3xl border border-gray-100 shadow-2xl shadow-gray-200/80 transform transition-all duration-300">
        ...
        <div class="border-b border-gray-100 pb-5 mb-6">
            <span class="text-xs font-bold text-green-600 uppercase tracking-widest bg-green-50 px-3 py-1 rounded-full">Update Mode</span>
            <h2 class="text-2xl font-extrabold text-gray-900 tracking-tight mt-3">Ubah Data Komoditas</h2>
            <p class="text-xs text-gray-400 mt-1">Pastikan spesifikasi hewan qurban diisi dengan akurat.</p>
        </div>
        ...
        <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            @if ($errors->any())
                <div class="bg-red-50 border border-red-200 text-red-700 p-4 rounded-xl text-xs space-y-1 shadow-sm">
                    <p class="font-bold mb-1"><i class="fa fa-exclamation-triangle mr-1"></i> Gagal menyimpan perubahan:</p>
            ...
            <div class="group">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="name" value="{{ old('name', $product->name) }}" class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl focus:border-green-600 text-sm outline-none transition-all duration-200 form-input-focus text-gray-700 font-medium" required>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div class="group">
                    ...
                    <div class="relative">
                        ...
                        <select name="type" class="w-full pl-10 pr-4 py-3 border border-gray-200 bg-white rounded-xl focus:border-green-600 text-sm outline-none transition-all duration-200 form-input-focus text-gray-700 font-medium cursor-pointer appearance-none" required>
                            <option value="Sapi" {{ $product->type == 'Sapi' ? 'selected' : '' }}>Sapi</option>
                            <option value="Kambing" {{ $product->type == 'Kambing' ? 'selected' : '' }}>Kambing</option>
                            <option value="Domba" {{ $product->type == 'Domba' ? 'selected' : '' }}>Domba</option>
                        </select>
                ...
                <div class="group">
                    <label class="block text-xs font-bold uppercase text-gray-500 mb-1.5 tracking-wider group-focus-within:text-green-600">Harga Penjualan (Rp)</label>
                    <div class="relative">
                        ...
                        <input type="number" name="price" value="{{ old('price', $product->price) }}" class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl focus:border-green-600 text-sm outline-none transition-all duration-200 form-input-focus text-gray-700 font-bold text-green-600" required>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div class="group">
                    ...
                    <div class="relative">
                        <input type="number" name="weight" value="{{ old('weight', $product->weight) }}" class="w-full pl-4 pr-10 py-3 border border-gray-200 rounded-xl focus:border-green-600 text-sm outline-none transition-all duration-200 form-input-focus text-gray-700 font-medium" required>
                ...
                <div class="group">
                    ...
                    <div class="relative">
                        <input type="number" name="age" value="{{ old('age', $product->age) }}" class="w-full pl-4 pr-14 py-3 border border-gray-200 rounded-xl focus:border-green-600 text-sm outline-none transition-all duration-200 form-input-focus text-gray-700 font-medium" required>
                ...
                <div class="group">
                    <label class="block text-xs font-bold uppercase text-gray-500 mb-1.5 tracking-wider transition-colors group-focus-within:text-green-600">Status Ketersediaan</label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center text-gray-400 group-focus-within:text-green-600 transition-colors">
                            <i class="fa fa-info-circle text-sm"></i>
                        </span>
                        <select name="is_sold" class="w-full pl-10 pr-4 py-3 border border-gray-200 bg-white rounded-xl focus:border-green-600 text-sm outline-none transition-all duration-200 form-input-focus text-gray-700 font-medium cursor-pointer appearance-none" required>
                            <option value="0" {{ !$product->is_sold ? 'selected' : '' }}>Tersedia</option>
                            <option value="1" {{ $product->is_sold ? 'selected' : '' }}>Terjual</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="group">
                <label class="block text-xs font-bold uppercase text-gray-500 mb-1.5 tracking-wider group-focus-within:text-green-600">📍 Lokasi Peternakan (Kab/Kota)</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center text-gray-400 group-focus-within:text-green-600">
                        <i class="fa fa-map-marker-alt text-sm"></i>
                    </span>
                    <input type="text" name="location" value="{{ old('location', $product->location) }}" class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl focus:border-green-600 text-sm outline-none transition-all duration-200 form-input-focus text-gray-700 font-medium" required>
                </div>
            </div>

            <div class="group">
                <label class="block text-xs font-bold uppercase text-gray-500 mb-1.5 tracking-wider group-focus-within:text-green-600">🩺 Surat / Keterangan Sehat</label>
                <div class="relative">
                    <span class="absolute left-3.5 top-3.5 text-gray-400 group-focus-within:text-green-600">
                        <i class="fa fa-heartbeat text-sm"></i>
                    </span>
                    <textarea name="health_status" rows="3" class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl focus:border-green-600 text-sm outline-none transition-all duration-200 form-input-focus text-gray-700 leading-relaxed" required>{{ old('health_status', $product->health_status) }}</textarea>
                </div>
            </div>
            ...
            <div class="bg-gray-50 p-4 rounded-2xl border border-gray-100 space-y-3">
                ...
                @if($product->image)
                    <div id="old-image-preview" class="flex items-center gap-4 bg-white p-2.5 rounded-xl border border-gray-200 shadow-sm w-fit transition-all duration-200">
                        <img src="{{ \Illuminate\Support\Facades\Storage::url($product->image) }}" class="w-24 h-16 object-cover rounded-lg border border-gray-100 shadow-inner">
                        <div>
                            <span class="text-xs font-bold text-gray-700 block">Foto Saat Ini Aktif</span>
                        </div>
                    </div>
                @endif
                <div id="dropzone" class="relative bg-white border-2 border-dashed border-gray-200 hover:border-green-500 rounded-xl p-4 text-center transition-all duration-200 cursor-pointer group/file">
                    <input type="file" name="image" id="image-input" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" accept="image/*">
                    <div id="upload-instruction" class="space-y-1">
                        <i class="fa fa-cloud-upload-alt text-gray-400 group-hover/file:text-green-600 text-2xl transition-colors"></i>
                        <p class="text-xs font-semibold text-gray-600">Klik untuk unggah atau seret file gambar baru</p>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-gray-100 mt-6">
                ...
                <button type="submit" class="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3.5 px-4 rounded-xl transition-all duration-200 text-sm shadow-xl shadow-green-600/10 hover:shadow-green-600/20 active:scale-[0.99] flex items-center justify-center gap-2">
                    <i class="fa fa-check-circle text-base"></i> Simpan Seluruh Perubahan
                </button>
            </div>
        </form>
    </div>
        ...
        function handleFile(files) {
            ...
            if (files.length > 0) {
                ...
                if (file.size > MAX_FILE_SIZE) {
                    ...
                    if (oldImagePreview) oldImagePreview.classList.remove('opacity-40', 'scale-95');
                    return;
                }
                ...
                reader.onload = function(e) {
                    ...
                    if (oldImagePreview) oldImagePreview.classList.add('opacity-40', 'scale-95');
                }
